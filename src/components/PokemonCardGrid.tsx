                    ...
                    return (
                        <div className="pokemon-card" key={data.id}>
                            <div className="pokemon-card-list">
                                {location.pathname.includes("/pokemon") || location.pathname.includes("/search") ? (<FaPlus onClick={()=>dispatch(addPokemonToList(data))} className='plus' />) :  <FaTrash onClick={async ()=> {await dispatch(removePokemon({id : data.firebaseId!})); dispatch(setToast("Pokemon Removed Successfully."));}} className='trash'/>}
                            </div>
                            ...
                            <img src={data.image} alt={data.name} onClick={() => {navigate(`/pokemon/${data.id}`); dispatch(setPokemonTab(pokemonTabs.description));dispatch(setCurrentPokemon(undefined));}} className='pokemon-card-image' loading='lazy'/>
